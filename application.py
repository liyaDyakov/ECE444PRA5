...
<html lang="en">
  ...
  <body>
    ...
    <div class="info">
      <strong>Model Loaded:</strong> {{ 'Yes' if model_loaded else 'No' }}<br>
      <strong>Model Path:</strong> {{ model_path }}
    </div>

    <form action="/predict-form" method="post">
      <label for="message"><strong>Enter text to classify:</strong></label>
      <textarea id="message" name="message" placeholder="Type your text here..."></textarea>
      <button type="submit">Predict</button>
    </form>

    {% if prediction %}
    <div class="prediction {% if prediction|lower == 'fake' %}fake{% endif %}">
